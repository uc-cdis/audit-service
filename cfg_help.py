...
LOCAL_CONFIG_FOLDER = "{}/.gen3/audit-service".format(expanduser("~"))
...
def main():
    ...
    create.add_argument(
        "-n",
        "--name",
        default="audit-service-config.yaml",
        help=(
            "configuration file name if you want something "
            'other than "audit-service-config.yaml"'
        ),
    )
    create.add_argument(
        "--config_path",
...
def create_config_file(file_name, full_path=None):
    ...
    copyfile(os.path.join(ROOT_DIR, "src/audit/config-default.yaml"), config_path)
